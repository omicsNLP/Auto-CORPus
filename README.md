...
40289: character maps to <undefined>'`. The same issue is not found on Linux or Mac machines. We are looking to resolve this problem, but in the meantime, there is a workaround: when saving a webpage using the internet browser (such as Microsoft Edge), change the dropdown from "complete" to "HTML".
