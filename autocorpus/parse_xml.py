...
def read_mapping_file():
    ...
    with open('./data/IAO_FINAL_MAPPING.txt', 'r', encoding='utf-8') as f:
...
def read_IAO_term_to_ID_file():
    ...
    with open('./data/IAO_term_to_ID.txt', 'r') as f:
        # Read all lines from the file
        lines = f.readlines()
...
def extract_section_content(section, soup2, ori_title):
    ...
    if curren_title != None:
        target_title = soup2.find('title', string=f"{curren_title.text}")  # Find the exact title in the soup2 object
...
for xyz in range(len(all_files)):
    ...
    try:
        ...
        if re.sub('<[^>]+>', '', str(soup.find('article-categories').find('subject'))) != 'None':
            # If valid text exists, remove XML tags and extract the content
        ...
        if re.sub('<[^>]+>', '', str(soup.find('kwd-group'))) != 'None':
            ...
            text_list[1] = re.sub('</ital[^>]+>', '', re.sub('<ital[^>]+>', '', str(text_list[1])))
            text_list[1] = re.sub('</bol[^>]+>', '', re.sub('<bol[^>]+>', '', str(text_list[1])))
            ...
            for i in range(len(re.sub('<[^>]+>', ',', str(text_list[1])).split(','))):
                if re.sub('<[^>]+>', ',', str(text_list[1])).split(',')[i] != '':
                    current.append(re.sub('<[^>]+>', ',', str(text_list[1])).split(',')[i])
            ...
            if len(soup.find_all('abstract')) == 1:
                ...
                if len(soup.find('abstract').find_all('title')) > 0:
                    ...
                    for title in soup.find('abstract').find_all('title'):
                        ...
                        while next_sibling and next_sibling.name == 'p':  # Check for 'p' elements
                        ...
                        for p_tag in p_tags:
                            ...
                            tag_subtitle.append([title_text])  # Title text from the 'title' tag
                            text_list.append(p_tag.text)  # Content of the 'p' tag (paragraph)
        ...
        inlinesupplementarymaterial_to_remove = soup2.find_all('inline-supplementary-material')
        ...
        for a in range(len(sec_elements)):
            ...
            sec_elements[a] = BeautifulSoup(text_test, features = 'lxml').find_all('sec')[0] # we keep the 0 element because we want the paragraph as a all not specific section since the parsing is taking place after
        ...
        for ref in ref_tags:
            ...
            if ref.find('element-citation', {'publication-type': 'journal'}) is not None:
                ...
                article_title = ref.find('article-title').text if ref.find('article-title') else ''
                ...
                doi = ref.find('pub-id', {'pub-id-type': 'doi'}).text if ref.find('pub-id', {'pub-id-type': 'doi'}) else ''

                # Extract the PMID, if available
                pmid = ref.find('pub-id', {'pub-id-type': 'pmid'}).text if ref.find('pub-id', {'pub-id-type': 'pmid'}) else ''
                ...
                if ref.find('person-group', {'person-group-type': 'author'}) is not None:
                    ...
                    if len(author_group.find_all('name')) > 0:
                        for name in author_group.find_all('name'):
                            ...
                            given_names = name.find('given-names').text if name.find('given-names') else ''  # Extract given names, if available
                            authors.append(f"{given_names} {surname}")  # Append the author's name to the authors list
                ...
                text_list_ref.append(f"{label}{' ' + final_authors if final_authors else ''}{', ' + article_title if article_title else ''}{', ' + source if source else ''}{', ' + year if year else ''}{';' + volume if volume and year else ''}{', ' + volume if volume and not year else ''}")
        ...
        for i in range(len(tag_title)):
            ...
            tag_subtitle[i] = [re.sub('<[^>]+>', '', str(tag_subtitle[i])).replace("'", "").replace("[", "").replace("]", "").replace('"', '')]
            ...
            for j in range(len(tag_title[i])):
                ...
                # Remove closing square brackets from the string, removing the list syntax from the string
                # Remove double quotes from the string, removing the list syntax from the string
                tag_title[i][j] = [re.sub('<[^>]+>', '', str(tag_title[i][j])).replace("'", "").replace("[", "").replace("]", "").replace('"', '')]
            ...
            for j in range(len(tag_title[i])):
                ...
                if len(tag_title[i][j][0].split()) > 1:
                    ...
                    if tag_title[i][j][0].split()[0][-1] == '.':
                        ...
                        if tag_title[i][j][0].split()[0][:-1].replace('.', '').replace(',', '').isdigit():
        ...
        for i in range(len(tag_subtitle)):
            ...
            if len(tag_subtitle[i][0].split()) > 1:
                ...
                if tag_subtitle[i][0].split()[0][-1] == '.':
                    # Check if the first word (excluding the period) is a number (ignoring commas and periods)
                    if tag_subtitle[i][0].split()[0][:-1].replace('.', '').replace(',', '').isdigit():
                        # Remove the first word (likely a number followed by a period) and join the remaining words
        ...
        for i in range(len(text_list)):
            ...
            text_list[i] = re.sub('<sec[^*]+/title>', '', str(text_list[i])).replace('</sec>', '') 
            ...
            if text_list[i] != '':
                ...
                for j in range(len(text_list[i].split('<p>'))):
                    ...
                    if text_list[i].split('<p>')[j].replace('</p>', '') != '':
                        ...
                        new_text = str(re.sub('<[^>]+>', '', str(text_list[i].split('<p>')[j])))
                        ...
                        new_text = new_text.replace('</p>', '').replace('&lt;', '<').replace('&gt;', '>').replace('&amp;', '&').replace('&apos;', "'").replace('&quot;', '"')
        ...
        for y in range(len(corrected_section)):
            if corrected_section[y][0][0] == 'document title':
                ...
                tokenized_section_heading = nltk.wordpunct_tokenize(corrected_section[y][0][0])
                            ...
                            for IAO_term, heading_list in mapping_dict.items():
                                for i in range(len(heading_list)):
                                    if fuzz.ratio(h2_part, heading_list[i]) > 80:
                                        mapping_result.append(__add_IAO(heading_list[i], IAO_term))
        ...
        for y in range(len(tag_title_ref)):
            if tag_title_ref[y][0] == 'document title':
                ...
                tokenized_section_heading = nltk.wordpunct_tokenize(tag_title_ref[y][0])
                text = nltk.Text(tokenized_section_heading)
                words = [w.lower() for w in text]
                h2_tmp = ' '.join(word for word in words)
        ...
        for i in range(len(corrected_section)):
            ...
            if len(corrected_section[i]) > 1:
                for imp in range(1,len(corrected_section[i])):
                    embeded_dict[f'section_title_{cont_section}'] = corrected_section[i][imp][0]
                    cont_section += 1
        ...
        for i in range(len(tag_title_ref)):
            embeded_dict = {
                'section_title_1' : tag_title_ref[i][0] # First section title i.e. 'Reference'
                }
        ...
        with open(f'./xml_hackathon/{all_files[xyz].split("/")[-1].split(".")[0]}.json', 'w', encoding="utf-8") as fp:
