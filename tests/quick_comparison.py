    ...
    if folder2_file_path.exists():
        with open(folder1_file_path) as f1, open(folder2_file_path) as f2:
            ...
            different_lines = [
                ...
                if re.sub(r"\s+", "", line1, flags=re.UNICODE)
                != re.sub(r"\s+", "", line2, flags=re.UNICODE)
                and not [x for x in lines_to_ignore if x in line1]
            ]
            false_positives = different_lines
            different_lines = []
            for i in range(len(false_positives)):
                if (
                    "[PMC free article]" not in lines1[false_positives[i]]
                    and "[PMC free article]" in lines2[false_positives[i]]
                ):
                    continue
                else:
